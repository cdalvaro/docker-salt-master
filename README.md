...
# Dockerized SaltStack Master v3000.5
...
```sh
docker pull cdalvaro/saltstack-master:3000.5
```
...
docker run --name salt_master --detach \
    ...
    cdalvaro/saltstack-master:3000.5
```
...
docker run --name salt_stack -it --rm \
    --volume $(pwd)/keys/:/home/salt/data/keys/ \
    cdalvaro/saltstack-master:3000.5 app:gen-signed-keys new_master_sign
```
...
services:
  master:
    ...
    image: cdalvaro/saltstack-master:3000.5
    healthcheck:
